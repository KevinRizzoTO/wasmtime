...
#[test]
fn compiles_with_winch_for_multiple_args() -> Result<()> {
    let mut c = Config::new();
    ...
    let f = instance.get_func(&mut store, "sum").ok_or(anyhow::anyhow!("test function not found"))?;

    let mut returns = vec![Val::null(); 1];

    // Winch doesn't support calling typed functions at the moment.
    f.call(&mut store, &[Val::I32(2), Val::I32(2)], &mut returns)?;

    assert_eq!(returns[0].unwrap_i32(), 4);
